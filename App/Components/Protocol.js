...
export default class Protocol extends PureComponent {
  ...
  getFeeds = async () => {
    try {
      ...
      RNFetchBlob.fetch(
        'POST',
        mainDomain2 + 'getBooksProtocols.php',
        {
        ...
        [
          ...
          // { name: 'teacherId', data: this.state.id.toString() },
        ],
      )
        .then(res => {
  ...
  renderRow(item, index) {
    const image =
      item.item.image != ''
        ? {
            uri: newsAssetsDomain + item.item.image,
          }
        : require('./../Assets/feed.png');
    return (
      <TouchableOpacity
        onPress={() => this.details(item, image)}
        style={styles.container}>
        <View style={styles.itemImage}>
          <Image source={image} style={styles.feedImg} />
        </View>
        <View style={styles.itemText}>
          <Text style={styles.title}>{item.item.titre}</Text>
          <Text style={styles.content}>{item.item.article}</Text>
        </View>
      </TouchableOpacity>
    );
  }
